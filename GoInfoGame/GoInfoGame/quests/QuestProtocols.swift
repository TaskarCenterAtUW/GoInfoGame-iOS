    ...
    func updateUndoTags(changeSet: StoredChangeset) {
        ...
        Task.detached(operation: { @MainActor in
            ...
            do {
                ...
                case .node:
                    result = try await DatasyncManager.shared.syncNode(node: changeSet.asOSMNode(isUndo: true), exclude_gig_tags: true, editedTags: changeSet.tags.toDictionary(), checkConflicts: false)
                case .way:
                    result = try await DatasyncManager.shared.syncWay(way: changeSet.asOSMWay(isUndo: true), exclude_gig_tags: true, editedTags: changeSet.tags.toDictionary(), checkConflicts: false)
                case .unknown:
