        ...
        for node in nodeElements {
            ...
            for quest in allQuests {
                ...
                if quest.isApplicable(element: node){
                    ...
                    let unit = DisplayUnitWithCoordinate(displayUnit: duplicateQuest.displayUnit, coordinateInfo:  CLLocationCoordinate2D(latitude: node.position.latitude, longitude: node.position.longitude), id: node.id, subheading: "")
        ...
        for way in wayElements{
            for quest in allQuests {
                ...
                if quest.isApplicable(element: way){
                    ...
                    let unit = DisplayUnitWithCoordinate(displayUnit: duplicateQuest.displayUnit, coordinateInfo: position, id: way.id, subheading: "")
                    displayUnits.append(unit)
